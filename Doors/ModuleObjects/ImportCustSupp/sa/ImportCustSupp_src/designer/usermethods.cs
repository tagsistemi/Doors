...
namespace Customization.Doors.Doors.DynamicDocuments.ImportCustSupp.ImportCustSupp
{
	...
	{
		public void tile_MTileDialog_ctrl_MCheckBox_CheckedChanged(Object sender, EasyBuilderEventArgs e)
		{
			try
			{
				this.view_Dynamic.tileManager_MTileManager.tileGroup_MTileGroup1.tile_MTileDialog1.ctrl_DBTElencoCF.Enabled = false;
				for(int i=0;i<Document.DBTElencoCF.RowsCount;i++)
				{
					TDBTElencoCF_EB_LocalData riga = Document.DBTElencoCF.GetRow(i);
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Sel.Value = Document.DBTImpCF.TEB_LocalData.fld_Selezione.Value;
				}
				this.view_Dynamic.tileManager_MTileManager.tileGroup_MTileGroup1.tile_MTileDialog1.ctrl_DBTElencoCF.Enabled = true;
			}
		...
		public void DocumentController_BatchExecute(Object sender, ControllerEventArgs e)
		{
			try
			{
				if(Document.DBTElencoCF.RowsCount > 0)
				{
					for(int i=0;i<Document.DBTElencoCF.RowsCount;i++)
					{
						TDBTElencoCF_EB_LocalData riga = Document.DBTElencoCF.GetRow(i);
						if(riga.fld_Sel.Value)
						{
							if(Document.DBTImpCF.TEB_LocalData.fld_Clienti.Value)
							{
								Document.DBTElencoCF.CastToMyRecord(riga).fld_NoteElab.Value = InsCliente(riga);
							}
							else
							{
								Document.DBTElencoCF.CastToMyRecord(riga).fld_NoteElab.Value = InsFornitore(riga);
							}
						}
					}
				}
				...
				{
					MessageBox.Show("Niente da elaborare!", "Importa Clienti/Fornitori", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
			}
			catch (System.Exception exc)
			{
		...
		public string InsCliente(TDBTElencoCF_EB_LocalData riga)
		{
			string res = "Cliente inserito correttamente con Codice: ";
			//trovo l'ultimo codice cliente
			string codcli="";
			if(Document.DMCustSuppParameters.Read()== MDataManager.ReadResult.Found)
			{
				codcli=(Document.DMCustSuppParameters.TMA_CustSuppParameters.fld_LastCustomer.Value+1).ToString();
			}
			Document.DMCustSuppParameters.Close();			

			try
			{
				MDCustomers cli = MDCustomers.CreateUnattended(Document.Context);
				cli.EnterInNewRecord();
				
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_CustSupp.Value = codcli;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_CompanyName.Value = riga.fld_RagSoc.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_TaxIdNumber.Value = riga.fld_PartitaIva.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_ZIPCode.Value = riga.fld_CAP.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_City.Value = riga.fld_Citta.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_EMail.Value = riga.fld_EMail.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_Address.Value = riga.fld_Indirizzo.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_County.Value = riga.fld_Provincia.Value;
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_ISOCountryCode.Value = "IT";
				cli.DBTCustomersSuppliers.TMA_CustSupp.fld_Telephone1.Value = riga.fld_Telefono.Value;
				
				//aggiungo la sede di spedizione
				if(riga.fld_DestMerce.Value != "")
				{
					BusinessObjects.ERP.CustomersSuppliers.Documents.Customers.TDBTOtherBranches_MA_CustSuppBranches sedeDest = cli.DBTOtherBranches.AddRow();
					cli.DBTOtherBranches.CastToMyRecord(sedeDest).fld_Branch.Value = "001";
					cli.DBTOtherBranches.CastToMyRecord(sedeDest).fld_Address.Value = riga.fld_DestMerce.Value;
					cli.DBTOtherBranches.CastToMyRecord(sedeDest).fld_CompanyName.Value = riga.fld_RagSoc.Value;
				}
				
				if(!cli.SaveCurrentRecord())
				{
					cli.StartMessageSession("");
					System.Collections.Generic.List<string> msgCli = cli.GetAllMessages();
					cli.ShowMessage(true);
					
					foreach(string msg in msgCli)
					{
						res +=  msg.ToString();
					}
				}
				...
				{
					res += cli.DBTCustomersSuppliers.TMA_CustSupp.fld_CustSupp.Value + " - ";
					if(cli.MessageFound())
					{
						cli.StartMessageSession("");
						System.Collections.Generic.List<string> msgCli = cli.GetAllMessages();
						cli.ShowMessage(false);
				...
				cli.Close();//chiudo la BO Clienti
				cli.Dispose();
			}
			catch(Exception exc)
			{
		...
		public string InsFornitore(TDBTElencoCF_EB_LocalData riga)
		{
			string res = "Fornitore inserito correttamente con Codice: ";
			//trovo l'ultimo codice fornitore
			string codfor = "";
			if(Document.DMCustSuppParameters.Read()== MDataManager.ReadResult.Found)
			{
				codfor=(Document.DMCustSuppParameters.TMA_CustSuppParameters.fld_LastSupplier.Value+1).ToString();
			}
			Document.DMCustSuppParameters.Close();			

			try
			{
				MDSuppliers forn = MDSuppliers.CreateUnattended(Document.Context);
				forn.EnterInNewRecord();
				
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_CustSupp.Value = codfor;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_CompanyName.Value = riga.fld_RagSoc.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_TaxIdNumber.Value = riga.fld_PartitaIva.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_ZIPCode.Value = riga.fld_CAP.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_City.Value = riga.fld_Citta.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_EMail.Value = riga.fld_EMail.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_Address.Value = riga.fld_Indirizzo.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_County.Value = riga.fld_Provincia.Value;
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_ISOCountryCode.Value = "IT";
				forn.DBTCustomersSuppliers.TMA_CustSupp.fld_Telephone1.Value = riga.fld_Telefono.Value;
				
				//aggiungo la sede di spedizione
				if(riga.fld_DestMerce.Value != "")
				{
					BusinessObjects.ERP.CustomersSuppliers.Documents.Suppliers.TDBTOtherBranches_MA_CustSuppBranches sedeDest = forn.DBTOtherBranches.AddRow();
					forn.DBTOtherBranches.CastToMyRecord(sedeDest).fld_Branch.Value = "001";
					forn.DBTOtherBranches.CastToMyRecord(sedeDest).fld_Address.Value = riga.fld_DestMerce.Value;
					forn.DBTOtherBranches.CastToMyRecord(sedeDest).fld_CompanyName.Value = riga.fld_RagSoc.Value;
				}
				
				if(!forn.SaveCurrentRecord())
				{
					forn.StartMessageSession("");
					System.Collections.Generic.List<string> msgCli = forn.GetAllMessages();
					forn.ShowMessage(true);
				...
				{
					res += forn.DBTCustomersSuppliers.TMA_CustSupp.fld_CustSupp.Value + " - ";
					if(forn.MessageFound())
					{
						forn.StartMessageSession("");
						System.Collections.Generic.List<string> msgCli = forn.GetAllMessages();
						forn.ShowMessage(false);
				...
				forn.Close();//chiudo la BO Clienti
				forn.Dispose();
			}
			catch(Exception exc)
			{
		...
		public void tile_MTileDialog_ctrl_MPushButton_Click(Object sender, EasyBuilderEventArgs e)
		{
			try
			{
				svuotaCorpo();
				...
				string percorsoFile=System.IO.Path.GetDirectoryName(Document.DBTImpCF.TEB_LocalData.fld_NomeFile.Value);
				string nomeFile=System.IO.Path.GetFileName(Document.DBTImpCF.TEB_LocalData.fld_NomeFile.Value);
	
				//tolgo le virgolette dal file che creano problemi ** 02/08/2021
				System.Text.UTF8Encoding utf8 = new System.Text.UTF8Encoding();
				string str = System.IO.File.ReadAllText(Document.DBTImpCF.TEB_LocalData.fld_NomeFile.Value, utf8);
				
				str = str.Replace("\"", " ");
				System.IO.File.WriteAllText(Document.DBTImpCF.TEB_LocalData.fld_NomeFile.Value, str, utf8);
				// ** 02/08/2021
				
				//Creo lo schema per il file csv
				//string[] lines = {"["+ nomeFile +"]", "ColNameHeader=False", "Format=Delimited(;)", "CharacterSet=ANSI",
				string[] lines = {"["+ nomeFile +"]", "ColNameHeader=True", "Format=Delimited(;)", "CharacterSet=65001",
				"Col1='col1' Char Width 255", "Col2='col2' Char Width 255", "Col3='col3' char Width 255", "Col4='col4' char Width 255", 
				"Col5='col5' char Width 255", "Col6='col6' char Width 255", "Col7='col7' char Width 255", "Col8='col8' char Width 255", 
				"Col9='col9' char Width 255", "Col10='col10' Char Width 255", "Col11='col11' char Width 255", "Col12='col12' char Width 255", 
				"Col13='col13' char Width 255",	"Col14='col14' char Width 255", "Col15='col15' char Width 255", "Col16='col16' char Width 255", 
				"Col17='col17' char Width 255", "Col18='col18' Char Width 255", "Col19='col19' char Width 255", "Col20='col20' char Width 255", 
				"Col21='col21' char Width 255",	"Col22='col22' char Width 255", "Col23='col23' char Width 255", "Col24='col24' char Width 255",
				"Col25='col25' char Width 255", "Col26='col26' char Width 255", "Col27='col27' char Width 255"};
				
	         		System.IO.File.WriteAllLines(@percorsoFile+ "\\schema.ini", lines);
				
				OleDbConnection conn = new OleDbConnection(
				    "Provider=Microsoft.Jet.OLEDB.4.0;Data Source="+@percorsoFile+";" + 
				    "Extended Properties=\"text;HDR=No;FMT=CsvDelimited\"");
				
				OleDbDataAdapter adapter = new OleDbDataAdapter("select * from " + nomeFile, conn);
				...
				{
					i++;
					TDBTElencoCF_EB_LocalData riga = Document.DBTElencoCF.AddRow();
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Sel.Value = true;	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_RagSoc.SetValue(row[0].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Indirizzo.SetValue(row[1].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Citta.SetValue(row[2].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_CAP.SetValue(row[3].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Provincia.SetValue(row[4].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_Telefono.SetValue(row[5].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_EMail.SetValue(row[7].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_PartitaIva.SetValue(row[8].ToString());	
					Document.DBTElencoCF.CastToMyRecord(riga).fld_DestMerce.SetValue(row[9].ToString());	
				}
				
				if(i>0)
				{
					Document.DBTImpCF.TEB_LocalData.fld_Selezione.Value = true;
				}
		...
		public void view_Dynamic_tileGroup_MTileGroup_tile_MTileDialog_ctrl_NomeFile_StateButton_StateButtonClicked(Object sender, MStateObjectEventArgs e)
		{
			try
			{
				OpenFileDialog open = new OpenFileDialog();
				open.Filter = "csv files (*.csv)|*.csv|txt files (*.txt)|*.txt|All files (*.*)|*.* ";
				open.Title = "Selezionare il file da importare";
				if (open.ShowDialog() == DialogResult.OK)
				{
				    Document.DBTImpCF.TEB_LocalData.fld_NomeFile.Value = open.FileName;
				}
			}
			catch (System.Exception exc)
			{
		...
		public void svuotaCorpo()
		{
			try
			{
				Document.DBTImpCF.TEB_LocalData.fld_Selezione.Value = false;
				while(Document.DBTElencoCF.RowsCount > 0)
				{
					Document.DBTElencoCF.DeleteRow(0);
				}
			}
			catch(Exception exc)
			{
